...
`$rageclick` — cheap and high-precision, because here the gating event _is_
...
`$rageclick` gate as the only filter** — this monitor owns the _what they
did_ axis of the disjointness rule in `replay-vision-scanners-core`.
...
**`{{STUCK_MOMENTS}}`** is three to six places a user of _this_ product
