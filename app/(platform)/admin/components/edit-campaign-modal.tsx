...
import { Edit2, Calendar, Users, Telescope, FileCode, Sparkles, RefreshCw } from "lucide-react";
  ...
  const handleRegisterDiscordCommands = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        toast.success(data.message || "Successfully registered /link command with Discord!");
      } else {
        toast.error(data.error || "Failed to register Discord commands.");
      }
    } catch (err: any) {
      toast.error(err.message || "Network error registering commands.");
    } finally {
        ...
        <form onSubmit={onSave} className="space-y-4 pt-1">
          ...
          {editCampTab === "overview" ? (
            ...
            <div className="space-y-3.5">
              ...
              {/* Slash Command /link Setup */}
              <div className="p-3.5 rounded-xl border border-[#5865F2]/30 bg-[#5865F2]/5 space-y-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="size-6 rounded-lg bg-[#5865F2] flex items-center justify-center text-white shrink-0">
